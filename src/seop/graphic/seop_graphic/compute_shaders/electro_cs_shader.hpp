...
const char* ampere_cs_source = // B = I / 2πr
    ...
    void main()
    {
            ...
            // 2. 공식 대입 E = q / 4πεr² = 1/r² * 112,994,350,282 -> 보정 1/r² * 100
            vec3 E = vec3(0.0);
            ...
            E += dir_from_pos * 1 / (dis_from_pos * dis_from_pos);
            E += dir_to_neg * 1 / (dis_from_neg * dis_from_pos);

            // 3. 입자의 움직임 방향 = 전기장 방향 / 속도 = 전기장 세기  F = qE, a = qE / m (q = 1, m = 1 가정 )
            float speed_scale = 1e+12;
            vel = E * speed_scale;
            if(length(vel) > 300.0)
            {
