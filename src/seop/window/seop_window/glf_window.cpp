...
auto Glf_window::get_cursor_pos_normalized() -> math::Vec2
{
    ...
    float nx = static_cast<float>(pos.x_) - half_window_size_.x_;
    float ny = -static_cast<float>(pos.y_) + half_window_size_.y_;
    if (nx < -half_window_size_.x_ || nx > half_window_size_.x_ || ny < -half_window_size_.y_ ||
        ny > half_window_size_.y_) {
...
auto Glf_window::get_cursor_pos_ndc() -> math::Vec2
{
    ...
    float x_ndc = (2.0f * pos.x_) / window_size_.x_ - 1.0f;
    float y_ndc = 1.0f - (2.0f * pos.y_) / window_size_.y_;
...
{
    window_size_ = size;
    half_window_size_ = size / 2;
    inv_window_size_ = math::Vec2{1 / size.x_, 1 / size.y_};
}
...
auto Glf_window::window_size() const -> const math::Vec2 &
{
    return window_size_;
}

auto Glf_window::inv_window_size() const -> const math::Vec2 &
{
    return inv_window_size_;
}

auto Glf_window::half_window_size() const -> const math::Vec2 &
{
...
auto Glf_window::open_window() -> bool
{
    ...
    window_ = (glfwCreateWindow(static_cast<int>(window_size_.x_), static_cast<int>(window_size_.y_),
